...
from equidexflow.utils.visualization import PlotlySubplotsVisualizer
...
def main(args, cfg):
    ...
    # Start test
    results = test(model, test_loader, metrics, cfg.device, visualizer)

    # Print results
    print_results(test_loader, results)

    # Write xlsx
    log_results(args.logdir, test_loader, results)

    # Save plotly figure
    save_figure(args.logdir, visualizer)


def test(model, test_loader, metrics, device, visualizer):
    # Initialize
    model.eval()
    ...
    results = {key: np.full((len(pc_list_types), max_num_objs, num_rots), np.nan) for key in list(metrics.keys())}
...
def log_results(logdir, test_loader, results):
    # Get object types and object ids
    obj_types = test_loader.dataset.obj_types
    obj_idxs_types = test_loader.dataset.obj_idxs_types
    ...
    for key, result in results.items():
        with pd.ExcelWriter(os.path.join(logdir, f'{key}.xlsx')) as w:
            for obj_type, obj_idxs_objs, result_type in zip(obj_types, obj_idxs_types, result):
                df = pd.DataFrame(result_type[:len(obj_idxs_objs)], index=obj_idxs_objs)
                df.to_excel(w, sheet_name=obj_type)
